...
#  browser()
  ...
  spr.f.est <- function(log.p, out=FALSE, sub="med", spr0=NULL){
    ...
    tmp <- calc.rel.abund(sel,Fr,na,M,waa,maa,max.age=max.age,Pope=Pope,ssb.coef=ssb.coef)
...
#   ypr <- sum(rel.abund*waa*(1-exp(-sel*Fr))*exp(-M/2))
  ...
  spr0 <- sum(calc.rel.abund(sel,0,na,M,waa,maa,max.age=max.age,Pope=Pope,ssb.coef=ssb.coef)$spr)  
  tmp <- lapply(F.range, function(x) calc.rel.abund(sel,x,na,M,waa,maa,max.age=max.age,Pope=Pope,ssb.coef=ssb.coef))
...
calc.rel.abund <- function(sel,Fr,na,M,waa,maa,max.age=Inf,Pope=TRUE,ssb.coef=0){
  rel.abund <- rep(NA, na)
  rel.abund[1] <- 1
  for (i in 2:(na-1)) {
    rel.abund[i] <- rel.abund[i-1]*exp(-M[i-1]-sel[i-1]*Fr)
...
#    ypr1 <- ypr1*waa
  ...
  spr <- rel.abund*waa[1:na]*maa[1:na]*exp(-ssb.coef*(sel[1:na]*Fr+M[1:na])) # waaとmaaにNAが入る対馬マイワシのために、!is.na(waa)を追加
...
#              if (new.ssb!="hokke") thisyear.ssb <- sum(naa[,1,k]*waa[,1,k]*maa[,1,k],na.rm=T)*res0$input$unit.waa/res0$input$unit.biom else thisyear.ssb <- sum(res0$naa[,length(years)]*res0$input$dat$waa[,length(years)]*res0$input$dat$maa[,length(years)]*exp(-0.5*(res0$input$dat$M[,length(years)]+res0$faa[,length(years)])),na.rm=T)*res0$input$unit.waa/res0$input$unit.biom
...
#      if(ifelse(is.null(vpa.mode),TRUE, sum(years==fyears[i+1])==0|vpa.mode$rec=="recfun")){
      if(fyears[i+1]-min.age < start.year){
        ...
        if(ts==1){
#              if (new.ssb!="hokke") thisyear.ssb <- sum(naa[,i+1-min.age,k]*waa[,i+1-min.age,k]*maa[,i+1-min.age,k],na.rm=T)*res0$input$unit.waa/res0$input$unit.biom else thisyear.ssb <- sum(naa[,i-min.age,k]*waa[,i-min.age,k]*maa[,i-min.age,k]*exp(-0.5*M[,i-min.age,k]-0.5*faa[,i-min.age,k]),na.rm=T)*res0$input$unit.waa/res0$input$unit.biom
            if(is.null(res0$ssb.coef)) res0$ssb.coef <- 0
...
#            thisyear.ssb <- sum(res0$naa[,length(years)]*res0$input$dat$waa[,length(years)]*res0$input$dat$maa[,length(years)]*exp(-res0$ssb.coef*(res0$input$dat$M[,length(years)]+res0$faa[,length(years)])),na.rm=T)*res0$input$unit.waa/res0$input$unit.biom            
...
#  if (new.ssb!="hokke") ssb <- naa*waa*maa*res0$input$unit.waa/res0$input$unit.biom else {ssb <- naa*waa*maa*exp(-0.5*(M+faa))*res0$input$unit.waa/res0$input$unit.biom; ssb[,2:dim(ssb)[2],] <- ssb[,1:(dim(ssb)[2]-1),]; ssb[,1,] <- res0$naa[,length(years)]*exp(-0.5*res0$faa[,length(years)]-0.5*res0$input$dat$M[,length(years)])*res0$input$dat$waa[,length(years)]*res0$input$dat$maa[,length(years)]}
...
SR.est <- function(vpares,gamma1=0.01,er.log=TRUE,
...
#                   FUN=median, # 漁獲量の何を最大化するか？
                   ...
                   N=1000, # 加入変動の大きさ & stochastic計算するときの繰り返し回数
...
#        b <- exp(p[2])
    ...
    # HSを推定するための関数
    get.HS <- function(R,SSB,er.log,gamma1){
        reg0 <- lm(R~SSB-1)
        a0 <- reg0$coef
        b0 <- 0.9
        # hockey-stick
        ...
        for(i in 1:nrow(obj.data))
            {
#                obj.data$obj[i] <- optim(c(obj.data$log.a[i],obj.data$conv.b[i],log(0.5)),HS,R=R,SSB=SSB,
#                                         method="BFGS",MLE=TRUE,er.log=er.log,gamma=gamma1,
#                                         a=obj.data$log.a[i],b=obj.data$conv.b[i])$value
    ...
    if(what.est[1]==TRUE){
        ...
        if(is.boot>0){ # ブートストラップ
            for(i in 1:is.boot){
                ...
                tmp <- get.HS(R[rand],SSB[rand],er.log,gamma1)
                boot.HS[i,] <- unlist(tmp[c("a","b","r0")])
            }
            for(i in 1:length(R)){
                tmp <- get.HS(R[-i],SSB[-i],er.log,gamma1)
    ...
    get.stat <- function(fout,eyear=eyear,hsp=0){
        tmp <- as.numeric(fout$vssb[(nrow(fout$vssb)-eyear):nrow(fout$vssb),-1])
        lhs <- sum(tmp<hsp)/length(tmp)
        ...
        a$biom.CV <- a$ssb.sd/a$ssb.mean
        return(a)
    }

    get.stat2 <- function(fout,eyear=eyear,hsp=0){
        tmp <- (nrow(fout$vwcaa)-eyear):nrow(fout$vwcaa)
        nage <- dim(fout$naa)[[1]]
        tb <- fout$naa * fout$waa / 1000000
        tc <- fout$caa * fout$waa / 1000000
        ssb <- fout$naa * fout$waa *fout$maa / 1000000        
...
#            tmp3 <- do.call(future.vpa2,farg)
    ...
    trace.func <- function(farg,eyear,hsp=0,
                           fmulti=c(seq(from=0,to=0.9,by=0.1),1,seq(from=1.1,to=2,by=0.1),3:5,7)){
        trace.res <- NULL
        farg$outtype <- "FULL"
...
#          if(is.na(xx[2])) range.tmp[2] <- max(trace.BH[[kk]]$fmulti)*10
          ...
          ## ここでtraceを追加
          trace.BH[[kk]] <- rbind(trace.BH[[kk]],trace.func(farg.BH,eyear,hsp=b.BH),
                                  fmulti=tmp$minimum+c(-0.025,-0.05,-0.075,0,0.025,0.05,0.075))
...
#          if(is.na(xx[2])) range.tmp[2] <- max(trace.RI[[kk]]$fmulti)*10
          ...
          ## ここでtraceを追加
          trace.RI[[kk]] <- rbind(trace.RI[[kk]],trace.func(farg.RI,eyear,hsp=b.RI),
                                  fmulti=tmp$minimum+c(-0.025,-0.05,-0.075,0,0.025,0.05,0.075))
...
#                       res=res.SL,jack=jack.SL,boot=boot.SL),
    if(what.est[2]==TRUE)
        output$bh <- list(a=a.BH,b=b.BH,sd=sd.BH,
                       res=res.BH,Fhist=Fhist.BH,ofv=res.BH$value,
...
#    tmp$nLL <- output$ofv 
    ...
    output$summary <- output$summary0[c("a","b","sd","MSY.ssb.mean.ssb.mean",
                                        ...
                                        "B0.biom.mean.biom.mean")]
    colnames(output$summary) <- c("a","b","sd","SSB_MSY","B_MSY","U_MSY","MSY","B0(SSB)","B0(Biomass)")
    class(output) <- "SR"
    return(output)
}
...
plot.Kobe2 <- get.trend <- function(srres,UBdata=NULL,SR="hs",plot.history=FALSE,is.plot=FALSE,pickU="",pickB="",ylab.tmp="U/Umsy",xlab.tmp="SSB/SSBmsy"){
    ...
    if(is.null(dres$TC.MT)) dres$TC.MT <- as.numeric(colSums(dres$input$dat$waa * dres$input$dat$caa))
...
allplot <- function(res0,target="hs",N.unit=1,pngfile="../buri.png",detail.plot=1:3){
    dmodel <- res0$vpares
    summary <- res0$summary[rownames(res0$summary)=="hs",]
    res00 <- res0[target][[1]]
...
#    summary$Bban <- dmodel$Bban
    ...
    ## currentF projection
    arg.tmp <- res0$hs$farg
    arg.tmp$multi <- 1
    fout0 <- do.call(future.vpa2,arg.tmp) # currentFでの将来予測の結果
    fout1 <- res0$hs$fout[[1]] # Fmsyでの将来予測の結果
...
#        abline(v=aa,col=c("chartreuse3","orange","red"))
        ...
        par(mar=c(4.3,4.3,3,1))
        aa <- summary[c("SSB_MSY","Blimit","SSB_HS")]    
        fit.tmp <- plot.SR(res0,what.plot="hs",pick="SSB_MSY",xyscale=c(1.8,1.3),is.legend=FALSE)
#        abline(v=aa,col=c("chartreuse3","orange","red"))
        title("R vs SSB (only HS)")        
    ...
    if(2%in%detail.plot){
        ...
        a <- plot.Kobe2(res0,pickU="U_MSY",pickB="SSB_MSY",is.plot=T)
        ...
        points(rownames(res0$vpares$faa),res0$hs$fout[[1]]$faa[,1],type="b")
        legend("topleft",pch=c(1,20),legend=c("Current F","Past Fs"),col=c(1,"gray"))
        title("Current F (2013-2015)")
        ...
        title("SSB",line=0.5)
                                        ...
                                        # plot catch
        y <- as.numeric(colSums(dmodel$input$dat$caa * dmodel$input$dat$waa,na.rm=T)) * N.unit /1000000
        aa <- summary[c("MSY")]
        ...
        points(c(2017,2015,2015),summary[tmp],pch=c(3,2,1),col=c(1,1,2))
        points(c(2017,2015,2015),summary[tmp],pch=c(3,2,1),col=c(1,1,2))
        legend("topleft",col=c("chartreuse3",1,1,2,"blue","red"),bg="white",
               pch=c(NA,3,2,1,20,20),legend=c("MSY","ABC2017","ABC2015","newABC2015","Projection (Fcur)","Projection (Fmsy)"),ncol=2,cex=0.8,
               lty=c(1,NA,NA,NA,1,1))
    ...
    if(3%in%detail.plot){        
        ...
        tres0 <- res0$hs$trace[[1]]
        ssb <- res0$hs$trace[[1]]$ssb.mean/1000
...
#        else col.tmp <- c(brewer.pal(9,"Greens"),rev(brewer.pal(ifelse(ncol(tc)-9<3,3,ncol(tc)-9),"GnBu")))
        ...
        plot.SR(res0,what.plot="hs",pick="SSB_MSY",xyscale=c(1.8,1.3),
                is.legend=FALSE)    
        ...
        range1 <- range(res0$vpares$ssb)/1000
        range2 <- range(as.data.frame(res0$vpares$ssb)[as.character(year.tmp)])/1000


        ### plot of SSB
        ssb.max <- max(c(range1,summary$"SSB_MSY"),na.rm=T) *1.5 /1000
        tb3 <- tb2[which(ssb<ssb.max),]
        matplot(ssb,tb2,type="n",xlab="",ylab="Biomass (1000MT)",xaxs="i",yaxs="i",
        ...
        text(rep(ssb.max*0.93,ncol(tb)),x-diff(c(0,x))/2,
             paste(0:(ncol(tb2)-1),"y/o: ",round(apply(res0$vpares$input$dat$waa,1,mean),0),"g"),cex=1)

        matpoints(ssb,tb2[,1],type="l",lwd=2,col="gray",lty=3)
        points(x <- res0$vpares$ssb/1000,
               y <- res0$vpares$baa/1000,type="o",
        ...
        text(rev(x[1]),rev(y)[1],rev(colnames(x))[1],adj=0)
                                        #    browser()    
        abline(v=summary$"SSB_MSY"/1000,lty=2)
        abline(v=summary$"Blimit"/1000,lty=2)
        abline(v=summary$"SSB_HS"/1000,lty=2)            
        text(summary$"SSB_MSY"/1000,max(tb3)*1.1,
             paste("SSB_MSY=",format(round(summary$"SSB_MSY"/1000),big.mark=","),"",sep=""),adj=0)
        text(summary$"Blimit"/1000,max(tb3)*1.0,
             paste("SSB_limit=",format(round(summary$"Blimit"/1000),big.mark=","),
                   "",sep=""),adj=0)
        text(summary$"SSB_HS"/1000,max(tb3)*1.05,
             paste("SSB_HS=",format(round(summary$"SSB_HS"/1000),big.mark=","),
                   "",sep=""),adj=0)
                                        #    text(max(ssb)*0.8,max(tb[,1],na.rm=T)*1.05,"予測加入量",col=2)
        ## plot of SSB CV
        par(new=T)
        y <- res0$hs$trace[[1]]$ssb.CV
        ...
        wcatch <- as.numeric(colSums(res0$vpares$input$dat$caa * res0$vpares$input$dat$waa/1000000/1000,na.rm=T))
        matplot(ssb,tc2,type="n",,xaxs="i",yaxs="i",ylab="Catch (1000MT)",
        ...
        abline(v=summary$"Blimit"/1000,lty=2)
        abline(v=summary$"SSB_HS"/1000,lty=2)                    
        points(x <- as.numeric(res0$vpares$ssb)/1000,
        ...
        points(x <- apply(res0$hs$fout[[1]]$vssb,1,mean)[1:10]/1000,
               y <- apply(res0$hs$fout[[1]]$vwcaa,1,mean)[1:10]/1000,col=2,
               type="o",pch=20,lwd=3)
        text(rev(x)[1],rev(y)[1],
             paste("Projection ",rownames(res0$hs$fout[[1]]$vssb)[10],"(F_MSY)",sep=""),adj=-0.1,col=2)

        points(x <- apply(fout0$vssb,1,mean)[1:10]/1000,
               y <- apply(fout0$vwcaa,1,mean)[1:10]/1000,col="blue",type="o",pch=20,lwd=3)
        text(rev(x)[1],rev(y)[1],
             paste("Projection ",rownames(res0$hs$fout[[1]]$vssb)[10],"(F_current)",sep=""),adj=-0.1,col="blue")

        ## plot of catch CV
        par(new=T)
        y <- res0$hs$trace[[1]]$catch.CV
        ...
        tmp <- round(ssb*1000)>0 & !is.na(ssb)
        matplot(ssb,tres0$fmulti,type="n",ylab="Fishing efforts (current=1)",xaxs="i",yaxs="i",xlab="SSB(1000MT)",xlim=c(0,ssb.max),
...
plotyield <- function(res0,int.res=NULL,detail.plot=FALSE){
#    par(mfrow=c(2,1))
    arg.tmp <- res0$hs$farg
...
#    fout.tmp <- do.call(future.vpa2,arg.tmp)
    ...
    # average
    plot(x <- res0$hs$trace[[1]]$fmulti,y <- res0$hs$trace[[1]]$catch.mean,type="n",xlim=c(0,max(x)),
         xlab="Multiplier to current F",ylab="Catch (ton)",ylim=c(0,max(res0$hs$trace[[1]]$catch.det,y)))
    menplot(res0$hs$trace[[1]]$fmulti,cbind(res0$hs$trace[[1]]$catch.L10,res0$hs$trace[[1]]$catch.H10),
    ...
    points(x <- res0$hs$trace[[1]]$fmulti,y <- res0$hs$trace[[1]]$catch.mean,type="l",xlim=c(0,max(x)),
           xlab="Multiplier to current F",ylab="Catch (ton)",ylim=c(0,max(res0$hs$trace[[1]]$catch.det,y)))    
    points(fmax1 <- x[which.max(y)],y[which.max(y)],pch=20,col=1)    

    if(isTRUE(detail.plot)){    
    # geomean
        points(x <- res0$hs$trace[[1]]$fmulti,y <- res0$hs$trace[[1]]$catch.geomean,col=2,type="l",xlim=c(0,2))
    points(fmax2 <- x[which.max(y)],y[which.max(y)],pch=20,col=2)

    # median
        points(x <- res0$hs$trace[[1]]$fmulti,y <- res0$hs$trace[[1]]$catch.median,col=3,type="l",xlim=c(0,2))
        points(fmax3 <- x[which.max(y)],y[which.max(y)],pch=20,col=3)
    }

    # deteministic
    points(x <- res0$hs$trace[[1]]$fmulti,y <- res0$hs$trace[[1]]$catch.det,col=4,
           type="l",xlim=c(0,2))
    ...
    ### plot SSB
    plot(x <- res0$hs$trace[[1]]$fmulti,y <- res0$hs$trace[[1]]$ssb.mean,type="n",xlim=c(0,max(x)),
         xlab="Relative F (to current F)",ylab="SSB (ton)")
    menplot(res0$hs$trace[[1]]$fmulti,cbind(res0$hs$trace[[1]]$ssb.L10,res0$hs$trace[[1]]$ssb.H10),
    ...
    points(x <- res0$hs$trace[[1]]$fmulti,y <- res0$hs$trace[[1]]$ssb.mean,type="l",xlim=c(0,max(x)),
         xlab="Relative F (to current F)",ylab="SSB (ton)")
    points(fmax1,y[x==fmax1],pch=20,col=1)

    if(isTRUE(detail.plot)){        
        points(x <- res0$hs$trace[[1]]$fmulti,y <- res0$hs$trace[[1]]$ssb.geomean,col=2,type="l",xlim=c(0,2))
        points(fmax2,y[x==fmax2],pch=20,col=2)
    
        points(x <- res0$hs$trace[[1]]$fmulti,y <- res0$hs$trace[[1]]$ssb.median,col=3,type="l",xlim=c(0,2))
        points(fmax3,y[x==fmax3],pch=20,col=3)
    }

    points(x <- res0$hs$trace[[1]]$fmulti,y <- res0$hs$trace[[1]]$ssb.det,
           col=4,type="l")
