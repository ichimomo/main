...
#    SRdata <- get.SRdata(res.pma,years=1990:2000) # 特定の期間のデータだけを使う場合
    ...
    fres.BH <- future.vpa(res.pma,multi=1,nyear=50, start.year=2012, 
                          ...
                          rec.arg=list(a=BH.par$pars$a,b=BH.par$pars$b,
                                       sd=BH.par$pars$sigma,bias.correction=TRUE,
    ...
    MSY.HS <- est.MSY(res.pma,fres.HS4$input,nyear=50,N=500,PGY=c(0.9,0.95),B0percent=c(0.3,0.4))

    MSY.HS <- est.MSY(res.pma,fres.HS4$input,nyear=50,N=500,PGY=c(0.9,0.95),B0percent=c(0.3,0.4),trace.multi=seq(from=0,to=1,by=0.1))    

    # BYによるMSY管理基準値を計算する場合：BH関数を使った将来予測の結果を引数に入れる
    MSY.BH <- est.MSY(res.pma,fres.BH$input,nyear=50,N=1000,PGY=c(0.9,0.95),B0percent=c(0.3,0.4))
...
plot.futures <- function(fres.list,conf=c(0.1,0.5,0.9),target="SSB",legend.text=""){
    ...
    plot(0,max(unlist(aa)),type="n",xlim=as.numeric(range(unlist(sapply(aa,function(x) colnames(x))))),
         ylim=c(0,max(unlist(aa))),xlab="Year",ylab=target)
...
get.stat2 <- function(fout,unit.waa=1,eyear=NULL,hsp=NULL){
...
plot.SRdata <- function(SRdata){
    plot(SRdata$SSB,SRdata$R,xlab="SSB",ylab="R",xlim=c(0,max(SRdata$SSB)),ylim=c(0,max(SRdata$R)))
                                        ...
                                        # HSを推定するための関数
fit.HS <- function(SRdata,er.log=TRUE,gamma1=0.001,do.profile=TRUE){
    HS <- function(p,R,SSB,gamma=gamma1,er.log=er.log,MLE=FALSE,a=NULL,b=NULL){
        if(!is.null(a)) p[1] <- a 
        if(!is.null(b)) p[2] <- b
    ...
    R.tmp <- pred.HS(SSB=ssb.tmp,a=a.HS,b=b.HS,gamma=gamma1)
    pred.data <- data.frame(SSB=ssb.tmp,R=R.tmp)

    SRdata$pred.HS <- pred.HS(SRdata$SSB,a=a.HS,b=b.HS,gamma=gamma1)
    SRdata$resid <- log(SRdata$R) - log(SRdata$pred.HS)    

    return(list(pars=data.frame(a=a.HS,b=b.HS,sigma=exp(res.HS$par[3]),r0=r0.HS),
                res=res.HS,AICc=AICc,obj.data=obj.data,ofv.least.square=ofv.least.square,gamma=gamma1,
                func="HS",pred=pred.data,resid=SRdata$resid))
}

                                        # Beverton-Holt
fit.BH <- function(SRdata,er.log=TRUE){
    SSB <- SRdata$SSB
    R <- SRdata$R        
    BH <- function(p,R,SSB,er.log=er.log,MLE=FALSE){
        a <- exp(p[1])
    ...
    ssb.tmp <- seq(from=0,to=max(SSB)*1.3,length=100)
    R.tmp <- pred.BH(SSB=ssb.tmp,a=a.BH,b=b.BH)
    pred.data <- data.frame(SSB=ssb.tmp,R=R.tmp)

    return(list(pars=data.frame(a=a.BH,b=b.BH,sigma=exp(res.BH$par[3])),
                res=res.BH,AICc=AICc,func="BH",pred=pred.data))
}

fit.RI <- function(SRdata,er.log=TRUE){
    SSB <- SRdata$SSB
    R <- SRdata$R            
    RI <- function(p,R,SSB,er.log=er.log,MLE=FALSE){
        a <- exp(p[1])
    ...
    ssb.tmp <- seq(from=0,to=max(SSB)*1.3,length=100)
    R.tmp <- pred.RI(SSB=ssb.tmp,a=a.RI,b=b.RI)
    pred.data <- data.frame(SSB=ssb.tmp,R=R.tmp)
    
    return(list(pars=data.frame(a=a.RI,b=b.RI,sigma=exp(res.RI$par[3])),
                res=res.RI,AICc=AICc,func="RI",pred=pred.data))        
}
...
#            ssb.array[,,i] <- tmp$vssb
    ...
    trace <- trace.func(farg.tmp,eyear,hsp=Blimit,fmulti=trace.multi)

    xx <- which.max(trace$table$catch.median)+c(-1,1)
    ...
    trace$table <- rbind(trace$table,trace.func(farg.msy,eyear,hsp=Blimit,
...
#    cat(" SSB=",MSY$"ssb.mean","\n")    
    ...
    if(!is.null(PGY)){
        ...
        rownames(PGYstat) <- names(fout.PGY) <- paste("PGY",rep(PGY,each=2),rep(c("upper","lower"),length(PGY)),sep="_")
    }
    else{
        PGYstat <-  NULL
        }
...
get.perform <- function(fout0,Blimit=0,longyear=50,smallcatch=0.5,N=NULL,
                        shortyear=c(3,5,10)){
    stat1 <- get.stat(fout0,eyear=0,hsp=Blimit)[c("catch.mean","catch.CV","biom.mean","biom.CV","ssb.mean","lower.HSpoint")]
    stat2 <- get.stat2(fout0,eyear=0)
    ...
    # 漁獲量<0.5平均漁獲量の頻度
    stat1$catch.safe <- 1/mean(fout0$vwcaa[nrow(fout0$vwcaa),]<smallcatch*mean(fout0$vwcaa[nrow(fout0$vwcaa),]))
    ...
    stat1$eq.year <- years[min(which(diff(fout0$vssb[,1])/fout0$vssb[-1,1]<0.01))]
...
fit.SR <- function(SRdata,SR="HS",method="L2",AR=1,gamma=0.001,TMB=FALSE,hessian=FALSE){
  ...
  obj.f <- function(a,b,sigma,rho){
    ...
    if (method == "L2") {
      for (i in 1:N) {
        if (i==1) {
          obj <- c(obj,-0.5*log(2*pi)-log(sigma^2/(1-rho^2))-resid2[i]^2/(2*sigma^2/(1-rho^2)))
        } else {
          obj <- c(obj, -0.5*log(2*pi)-0.5*log(sigma^2)-resid2[i]^2/(2*sigma^2))
        }
      }
    } else {
      for (i in 1:N) {
        if (i==1) {
          obj <- c(obj,-log(2*sqrt(sigma^2/(1-rho^2)))-abs(resid2[i])/sqrt(sigma^2/(1-rho^2)))
        } else {
          obj <- c(obj, -log(2*sigma)-abs(resid2[i])/sigma)
        }
      }
    }
    obj <- -sum(obj) # exact likelihood
    return(obj)
  }
  
  if (SR == "HS") { 
    init <- c(log(median(rec/ssb)),0,-0.5)
    if (AR == 0) {
      obj.f2 <- function(x) obj.f(exp(x[1]),min(ssb)+(max(ssb)-min(ssb))/(1+exp(-x[2])),exp(x[3]),0)
    } else {
      obj.f2 <-  function(x) obj.f(exp(x[1]),min(ssb)+(max(ssb)-min(ssb))/(1+exp(-x[2])),exp(x[3]),1/(1+exp(-x[4])))
      init <- c(init,0)
    }
  } else {
    init <- c(log(median(rec/ssb)),-log(max(ssb)),-0.5)
    if (AR == 0) {
      obj.f2 <- function(x) obj.f(exp(x[1]),exp(x[2]),exp(x[3]),0)
    } else {
      obj.f2 <-  function(x) obj.f(exp(x[1]),exp(x[2]),exp(x[3]),1/(1+exp(-x[3])))
      init <- c(init,0)
    }
  }
  
  opt <- optim(init,obj.f2)
  if (SR=="HS") {# grid search
    init <- opt$par
    grids <- expand.grid(seq(min(log(rec/ssb)),max(log(rec/ssb)),length=10),
                         seq(-10,10,length=10))
    for (j in 1:nrow(grids)){
      init[1] <- grids[j,1]
      init[2] <- grids[j,2]
      opt2 <- optim(init,obj.f2)
      if (class(opt2)!="try-error") {
        if (opt2$value < opt$value) opt <- opt2
      }
    }
  }
  opt2 <- try(optim(opt$par,obj.f2,method="BFGS",hessian=hessian))
  if (class(opt2)!="try-error") opt <- opt2
  ...
  sigma <- exp(opt$par[3])
  rho <- ifelse(AR==0,0,1/(1+exp(-opt$par[4])))
  ...
  sigma <- ifelse(method=="L2",sqrt(mean(resid2^2)),mean(abs(resid2))*sqrt(2))
  Res$pars <- c(a,b,sigma,rho)
  ...
  if (method=="L2") {
    if (TMB) {
      ...
      params$log_sigma <- log(Res$pars[3])
      ...
      # grid search
      grids <- expand.grid(seq(min(log(rec/ssb)),max(log(rec/ssb)),length=10),
                           seq(-log(max(ssb)),-log(min(ssb)),length=10))
      for (j in 1:nrow(grids)) {
        params2 <- params
        params2$rec_loga <- grids[j,1]
        params2$rec_logb <- grids[j,2]
        params2$log_sigma <- opt$par[3]
        params2$logit_rho <- opt$par[4]
  ...
  names(Res$pars) <- c("a","b","sigma","rho")
...
# Hockey-stick
HS.recAR <- function(ssb,vpares,deterministic=FALSE,rec.resample=NULL,
                    rec.arg=list(a=1000,b=1000,gamma=0.01,sd=0.1, rho=0,
                                 resample=FALSE,resid=0, 
                                 bias.correction=TRUE)){
  if(is.null(rec.arg$gamma)) rec.arg$gamma <- 0.01
  if(is.null(rec.arg$bias.correction)) rec.arg$bias.correction <- TRUE
  rec.arg$resample <- FALSE #resamplingなし
  if(is.null(rec.arg$rho)) rec.arg$rho <- 0
  
  rec <- rec.arg$a*(ssb+sqrt(rec.arg$b^2+(rec.arg$gamma^2)/4)-sqrt((ssb-rec.arg$b)^2+(rec.arg$gamma^2)/4))
  
  resid <- ifelse (is.numeric(rec.resample),rec.resample[length(rec.resample)],
                   ifelse(is.null(rec.arg$resid),0,rec.arg$resid[length(rec.arg$resid)]))
  rec0 <- rec # 再生産関係からの予測値
  rec <- rec0*exp(rec.arg$rho*resid) # 自己相関込みの予測値
  new.resid <- log(rec/rec0)
  sigma2 <- sqrt(rec.arg$sd^2/(1-rec.arg$rho^2)) #rho込み平均補正用SD
  
  if(!isTRUE(deterministic)){
    if(isTRUE(rec.arg$bias.correction)){
        rec <- rec*exp(rnorm(length(ssb),-0.5*sigma2^2,rec.arg$sd))
        new.resid <- log(rec/rec0)+0.5*sigma2^2
      } else {
        rec <- rec*exp(rnorm(length(ssb),0,rec.arg$sd))
        new.resid <- log(rec/rec0)
      }
