...
#
# (*) macで読む場合は、
# source("rvpa0.92.r",encoding="shift-jis")
...
)
{
  ...
  if (term.F=="max") {  # 最高齢のFだけを推定する場合
    ...
    if (!is.null(add.p.est)) {add.p.est <- add.p.est + 1; p.init <- rep(p.init, length(add.p.est)+1)}  # add.p.estに数字があれば，その年齢を追加のパラメータとして推定する
  }
...
# core function for optimization
  ...
  # tuning
    ...
    for (i in 1:nindex){
      ...
      if (!isTRUE(intercept)){
        q[i] <- exp(mean(log(as.numeric(index[i,]))-log(as.numeric(abundance)),na.rm=TRUE))
        obj <- c(obj,index.w[i]*sum((log(as.numeric(index[i,]))-log(q[i]*as.numeric(abundance)))^2, na.rm=TRUE))
      }
      if (isTRUE(intercept)){
  ...
  res <- list(input=arglist, term.f=term.f, np=np, minimum=out$minimum, minimum.c=out$minimum.c, logLik=logLik, gradient=gradient, code=code, q=q, hessian=hessian, Ft=Ft, Fc.at.age=Fc.at.age, Fc.mean=Fc.mean, Fc.max=Fc.max, last.year=last.year, Pope=Pope, pred.index=pred.index, naa=naa, faa=faa, baa=baa, ssb=ssb, saa=saa)

  if (isTRUE(plot) & isTRUE(tune)){
    for (i in 1:nindex){
      Y <- years %in% plot.year
      if (!isTRUE(intercept)) Pred <- index[i,Y]/q[i] else Pred <- (index[i,Y]-q[i,2]*ti.scale[2])/(q[i,1]*ti.scale[1])
...
SR.est <- function(res, model="BH", method="log", scale=1){
...
SR.est <- function(res,model="BH",k=1,p.init=NULL,lower.limit=-25,scale=1,main=NULL,log=FALSE){
  SSB <- colSums(res$ssb,na.rm=TRUE)/scale
  R <- res$naa[1,]/scale
...
ref.F <- function(
  ...
  waa=res$input$dat$waa, # 仮定する生物パラメータ．直接の値を入れるか，年を指定するやり方のどちらでも動く。直接指定するほうが優先。
  maa=res$input$dat$maa,
  M=res$input$dat$M,
  M.year=2012, 
  waa.year=2012, # 年を指定して生物パラメータを仮定する場合．年の範囲の平均値が用いられる
  maa.year=2012,
  rps.year = 2001:2010, # Fmedの計算に使うRPSの年の範囲
...
#  sel <- as.numeric(na.omit(sel)) # 将来予測などのためにそのままにしとく
#  na <- length(sel)
  na <- sum(!is.na(sel))

#  waa <- as.numeric(na.omit(waa))[1:na]
#  maa <- as.numeric(na.omit(maa))[1:na]
#  M <- as.numeric(na.omit(M))[1:na]
#  if(is.null(waa)){
    waa <- apply(as.matrix(as.data.frame(waa)[as.character(waa.year)]),1,mean)
#  }
#  else{
#    waa <- as.numeric(na.omit(waa))[1:na]
#  }
...
#  if(is.null(M)){
    M <- apply(as.matrix(as.data.frame(M)[as.character(M.year)]),1,mean)
#  }
#  else{
#    M <-  as.numeric(na.omit(M))[1:na]
#  }
...
#  if(is.null(maa)){
    maa <- apply(as.matrix(as.data.frame(maa)[as.character(maa.year)]),1,mean)
#  }
#  else{
#    maa <- as.numeric(na.omit(maa))[1:na]
#  }
...
#   ypr <- sum(rel.abund*waa*(1-exp(-sel*Fr))*exp(-M/2))
  ...
  tmp <- lapply(F.range, function(x) calc.rel.abund(sel,x,na,M,waa,maa,max.age=max.age))
...
future.vpa <- function(res0,
                       ...
                       start.year=2011,
                       waa.year=2009:2011, # VPA結果から生物パラメータをもってきて平均する期間
                       maa.year=2009:2011,
                       M.year=2009:2011,
                       ...
                       ABC.year=2013, # ABC yearを計算する年。これ以前はcurrent Fで漁獲
                       ...
                       rec.arg=list(upper.ssb=Inf,upper.recruit=Inf,rps.year=2001:2010), # 加入の各種設定
...
#      fres.tmp <- future.vpa(res0=res0,
#                             currentF=currentF,nyear=nyear,
#                             multi.year=multi.year,
#                             ts=ts,                               
#                             start.year=start.year,waa.year=waa.year,maa.year=maa.year,
#                             rec.new=rec.new,ABC.year=ABC.year,
#                             recfunc=recfunc,rec.arg=rec.arg,
#                             Frec=NULL,pre.catch=pre.catch,
#                             N=ifelse(Frec$stochastic==TRUE,N,0),
#                             multi=x)
...
#                            rep(nage,length(years)+1),length(years)+1)
        ...
        if(is.na(naa[1,1,k])){
          if(fyears[1]-min.age < start.year){
            ...
            thisyear.ssb <- sum(naa[,1,k]*waa[,1,k]*maa[,1,k],na.rm=T)
          }
...
#      tmp <- forward.calc(faa[,,k],naa[,,k],M[,,k],rep(nage,length(fyears)),i+1,plus.group=plus.group)
#      browser()
...
plot.future <- function(fres0,ylim.tmp=NULL,xlim.tmp=NULL,vpares=NULL,what=c(TRUE,TRUE,TRUE),
                        label=c("Biomass","SSB","Catch"),is.legend=TRUE,add=FALSE){
  matplot2 <- function(x,add=FALSE,...){
    if(add==FALSE) matplot(rownames(x),x,type="l",lty=c(2,1,2),col=1,xlab="Year",...)
    if(add==TRUE) matpoints(rownames(x),x,type="l",lty=c(2,1,2),col=1,xlab="Year",...)    
  }
...
RPS.simple.rec <- function(ssb,vpares,
                           rec.arg=list(rps.year=1995:2011, # 点推定値のrpsを計算する期間
...
masaba.rec <- function(ssb,vpares,
                       rec.arg=list(rps.years=list(1970:1985,1986:2011), # year < Blim, year > Blim
