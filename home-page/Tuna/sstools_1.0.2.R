...
#    par(org.par)
  ...
  org.par <- par()
  par(mpg=c(1.8,0.3,0),tck=0.03,font.main=1,cex.main=1,ps=11)
...
#      legend(1,10,legend=repfile.legend,col=col.var,lty=lty.var,lwd=lwd.var,ncol=2)
  ...
  matplot(rowtapply(apply(faa[[1]]$faa.array,c(1,2),sum)),type="b",ylab="F",xlab="Year")
  ...
  if(multiplot){
    for(i in 2:len.rep){
      ...
      mtext(side=3,line=0.5,adj=0.1,
            paste2("** Average F at age (",repfile[i],") **"),outer=T)
    }
  }
...
#          browser()
      ...
      if(j%%3==0) mtext("Bubble plot of Pearson residuals, darkblue (obs<exp), white (obs>exp)",outer=T)
    }
}
