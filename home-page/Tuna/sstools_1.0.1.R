...
#  par(org.par)
  invisible(list(biom=biom,SPR=SPR,naa=naa,faa=faa,cpue=cpue,
                 nma=nma,alk=alk,repfile=repfile,
...
function(repfile,target.fleet=0,target.term=0,
                               future.res=NULL,len.sd=0,error.func="select",
...
#      }
  ...
  list(exp.rage=tmp.array,wcaa.array=wcaa.array,target.res=target.res,
...
function(replist,repfile.legend=NULL){
  ...
  par.tmp <- par()
#  par(mfrow=c(6,1),mar=c(4,20,1,1),las=1)
...
#      plot(x <- 1:nrep,LLdata[,i],type="b",xaxt="n",
#           xlab="",ylab="",xlim=c(0.5,max(x)+0.5))
#      axis(at=x,labels=rep.name,side=1)
      if(s%%2==1){
        par(mar=c(4,15,1,1))
      }
      else{
